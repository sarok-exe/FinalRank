const rnd = (): number => crypto.getRandomValues(new Uint32Array(1))[0] / 0xFFFFFFFF;
...
export class CubicBezier {
  ...
  private getX(t: number): number { return 3*t*Math.pow(1-t,2)*this.p1x + 3*Math.pow(t,2)*(1-t)*this.p2x + Math.pow(t,3); }
  private getY(t: number): number { return 3*t*Math.pow(1-t,2)*this.p1y + 3*Math.pow(t,2)*(1-t)*this.p2y + Math.pow(t,3); }
  private getXDerivative(t: number): number { return 3*Math.pow(1-t,2)*this.p1x - 6*t*(1-t)*this.p1x + 6*t*(1-t)*this.p2x - 3*Math.pow(t,2)*this.p2x + 3*Math.pow(t,2); }
  ...
  evaluate(x: number): number { if (x <= 0) { return 0; } if (x >= 1) { return 1; } return this.getY(this.solveT(x)); }
}

export class Particle {
  x = 0; y = 0; vx = 0; vy = 0; life = 0; maxLife = 0; size = 0; color = '#fff';
  reset(w: number, h: number, color: string): void {
    this.x = rnd() * w; this.y = rnd() * h;
    const a = rnd() * Math.PI * 2, s = rnd() * 1.5 + 0.5;
    this.vx = Math.cos(a) * s; this.vy = Math.sin(a) * s;
    this.maxLife = rnd() * 100 + 50; this.life = this.maxLife;
    this.size = rnd() * 2.5 + 0.5; this.color = color;
  }
  update(w: number, h: number, wx: number, wy: number, drag: number): void {
...
export class ParticleSystem {
  ...
  init(w: number, h: number, color: string): void { this.particles.forEach(p => { p.reset(w, h, color); }); }
  updateAndDraw(ctx: CanvasRenderingContext2D, w: number, h: number, wx: number, wy: number, drag: number, color: string, sizeMult: number): void {
...
export type LayerConfig = { color: string; opacity: number; speed: number; blendMode: GlobalCompositeOperation; customParams: Record<string, number>; }
...
export function renderLayerDispatch(
  ...
  const zoom = conf.customParams.zoom;
  const circles = Math.floor(conf.customParams.circles * dayFactor);
  const thickness = conf.customParams.thickness;
  ...
  switch (layerId) {
    ...
    case 'layer2': {
      ...
      if (varId.includes('var2')) {
        ...
        for (let i = 0; i <= sides; i++) { const a = (i * Math.PI * 2) / sides + t, wb = 1 + Math.sin(t * 3 + i * 5) * 0.12 * easeVal; const x = cx + Math.cos(a) * curRad * wb, y = cy + Math.sin(a) * curRad * wb; if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); }
        ...
        for (let d = 1; d <= circles; d++) { const sides = 3 + d, r = (rad / circles) * d; ctx.beginPath(); for (let s = 0; s <= sides; s++) { const a = (s * Math.PI * 2) / sides + t * (d % 2 === 0 ? 0.3 : -0.3); const x = cx + Math.cos(a) * r, y = cy + Math.sin(a) * r; if (s === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.stroke(); }
      }
      break;
    }
    case 'layer3': {
      ctx.strokeStyle = conf.color;
      const wc = conf.customParams.waves, freq = conf.customParams.frequency;
      if (varId.includes('var2')) {
        for (let w = 0; w < wc; w++) { ctx.beginPath(); const amp = 40 + w * 15 * easeVal; for (let y = 0; y < h; y += 8) { const x = cx + Math.sin(y * freq + t + w) * amp; if (y === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.stroke(); }
      } else if (varId.includes('var3')) {
      ...
      else { for (let wi = 0; wi < wc; wi++) { ctx.beginPath(); const amp = 30 + wi * 12 * dayFactor; for (let x = 0; x < w; x += 6) { const y = cy + Math.sin(x * freq + t + wi * 0.8) * amp * easeVal; if (x === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.stroke(); } }
      break;
    }
    case 'layer4': {
      if (particles) {
        const wx = conf.customParams.windX, wy = conf.customParams.windY + Math.sin(t) * 0.05;
        ...
        else if (varId.includes('var4')) particles.particles.forEach(p => { p.vx += (rnd() - 0.5) * 0.8; p.vy += (rnd() - 0.5) * 0.8; });
    ...
    case 'layer5': {
      ...
      if (varId.includes('var2')) { for (let c = 0; c < cols; c++) { const theta = t + (c * Math.PI * 2) / cols, dist = ((t * 220 + c * 30) % 350) * easeVal; ctx.fillText(rnd() > 0.5 ? '1' : '0', cx + Math.cos(theta) * dist, cy + Math.sin(theta) * dist); } }
    ...
    case 'layer6': {
      ctx.strokeStyle = conf.color;
      const sectors = conf.customParams.symmetry;
      if (varId.includes('var2')) {
        ...
        const drawBranch = (len: number, depth: number): void => { if (depth <= 0) { return; } ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, -len); ctx.stroke(); ctx.save(); ctx.translate(0, -len); ctx.rotate(0.4 * easeVal); drawBranch(len * 0.7, depth - 1); ctx.restore(); ctx.save(); ctx.translate(0, -len); ctx.rotate(-0.4 * easeVal); drawBranch(len * 0.7, depth - 1); ctx.restore(); };
      ...
      } else if (varId.includes('var4')) { ctx.beginPath(); for (let i = 0; i <= sectors * 2; i++) { const a = (i * Math.PI * 2) / (sectors * 2) + t * 0.1, rv = i % 2 === 0 ? rad * 0.8 : rad * 0.3 * (1 + Math.sin(t) * 0.2); const x = cx + Math.cos(a) * rv, y = cy + Math.sin(a) * rv; if (i === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.closePath(); ctx.stroke(); }
    ...
    case 'layer7': {
      ctx.strokeStyle = conf.color;
      const bScale = conf.customParams.scale;
      ...
      else if (varId.includes('var3')) { for (let j = 1; j <= 5; j++) { ctx.beginPath(); const ss = rad * 0.18 * j * bScale; for (let a = 0; a <= Math.PI * 2 + 0.1; a += 0.15) { const p = 1 + Math.sin(a * 4 + t + j) * 0.15 * easeVal; const x = cx + Math.cos(a) * ss * p, y = cy + Math.sin(a) * ss * p; if (a === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.stroke(); } }
      ...
      else if (varId.includes('var5')) { ctx.beginPath(); for (let a = 0; a <= Math.PI * 2 + 0.05; a += 0.04) { const j = Math.sin(a * 12 + t * 4) * 8 * easeVal; const x = cx + Math.cos(a) * (rad * 0.7 + j), y = cy + Math.sin(a) * (rad * 0.7 + j); if (a === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.closePath(); ctx.stroke(); }
      else { ctx.beginPath(); for (let a = 0; a <= Math.PI * 2 + 0.1; a += 0.1) { const rOff = 1 + Math.sin(a * 5 + t * 2) * 0.22 * easeVal; const x = cx + Math.cos(a) * rad * 0.7 * rOff, y = cy + Math.sin(a) * rad * 0.7 * rOff; if (a === 0) ctx.moveTo(x, y); else ctx.lineTo(x, y); } ctx.closePath(); ctx.stroke(); }
      break;
    }
    case 'layer8': {
      ctx.strokeStyle = conf.color; ctx.fillStyle = conf.color;
      const dotCount = Math.min(100, conf.customParams.dotCount + Math.floor(dayFactor * 15));
      const maxDist = conf.customParams.maxDistance * easeVal;
      ...
      else if (varId.includes('var3')) { for (let i = 0; i < dotCount; i += 6) { ctx.beginPath(); for (let m = 0; m < 6; m++) { const idx = (i + m) % dotCount; if (m === 0) ctx.moveTo(nodes[idx].x, nodes[idx].y); else ctx.lineTo(nodes[idx].x, nodes[idx].y); } ctx.closePath(); ctx.stroke(); } }
    ...
    case 'layer9': {
      const gf = conf.customParams.glitchFrequency;
      if (varId.includes('var2')) {
        ctx.strokeStyle = conf.color; const hexR = 30 * dayFactor, hStep = hexR * Math.sqrt(3), vStep = hexR * 1.5;
        for (let xc = -1; xc < w / hStep + 1; xc++) for (let yc = -1; yc < h / vStep + 1; yc++) { const shift = (yc % 2 === 0) ? hStep / 2 : 0, px = xc * hStep + shift, py = yc * vStep; if (Math.sin(xc + yc + t * 4) > (1 - gf)) { ctx.beginPath(); for (let s = 0; s <= 6; s++) { const a = (s * Math.PI) / 3; if (s === 0) ctx.moveTo(px + Math.cos(a) * hexR, py + Math.sin(a) * hexR); else ctx.lineTo(px + Math.cos(a) * hexR, py + Math.sin(a) * hexR); } ctx.stroke(); } }
      } else if (varId.includes('var3')) { ctx.fillStyle = conf.color; ctx.font = '9px monospace'; if (rnd() < 0.25) { ctx.fillText(`NODES:[${Math.floor(t*100)}]`, cx - 80, cy + 80); ctx.fillText(`DAY_${Math.floor(dayFactor*1000)}`, cx + 30, cy - 80); } }
      ...
      else { if (rnd() < gf) { const sh = rnd() * 25 + 5, sy = rnd() * (h - sh), off = (rnd() - 0.5) * 45 * easeVal; ctx.drawImage(ctx.canvas, 0, sy, w, sh, off, sy, w, sh); } }
      break;
    }
    case 'layer10': {
      ctx.strokeStyle = conf.color;
      const vs = conf.customParams.vortexSpeed * dayFactor;
      if (varId.includes('var2')) { for (let i = 0; i < 48; i++) { const a = (i * Math.PI) / 24 + t * 0.4, lf = 30 + Math.sin(t * 3 + i) * 20 * easeVal; ctx.beginPath(); ctx.moveTo(mx + Math.cos(a) * 70, my + Math.sin(a) * 70); ctx.lineTo(mx + Math.cos(a) * (70 + lf), my + Math.sin(a) * (70 + lf)); ctx.stroke(); } }
        ...
        const galaxyArms = (cxAngle: number): void => { ctx.beginPath(); for (let s = 1; s < 180; s += 3) { const a = s * 0.08 + t * 0.6 + cxAngle, dist = s * 1.4 * easeVal; if (s === 1) ctx.moveTo(cx + Math.cos(a) * dist, cy + Math.sin(a) * dist); else ctx.lineTo(cx + Math.cos(a) * dist, cy + Math.sin(a) * dist); } ctx.stroke(); };
        galaxyArms(0); galaxyArms(Math.PI);
      } else if (varId.includes('var4')) { ctx.beginPath(); for (let side = 0; side < w; side += 12) { const ss = cy - 80 + Math.sin(t * 1.5 + side * 0.01) * 35 + easeVal * 30; if (side === 0) ctx.moveTo(side, ss); else ctx.lineTo(side, ss); } ctx.stroke(); }
      ...
      else { ctx.beginPath(); for (let idx = 0; idx < 250; idx += 4) { const phi = idx * 0.08 + t * 1.4 * vs, dr = idx * 1.2 * easeVal; if (idx === 0) ctx.moveTo(mx + Math.cos(phi) * dr, my + Math.sin(phi) * dr); else ctx.lineTo(mx + Math.cos(phi) * dr, my + Math.sin(phi) * dr); } ctx.stroke(); }
      break;
    }
  }
}
